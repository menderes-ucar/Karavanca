...
class _ProductEditPageState extends State<ProductEditPage> {
  ...
  Future<void> _pickImage() async {
    ...
    final XFile? file = await picker.pickImage(
      ...
      imageQuality: 85, // kalite/byte dengesi
    );
    if (file == null) return;
    ...
    try {
      ...
      final path = 'products/$uid/${widget.id}/${DateTime.now().millisecondsSinceEpoch}.jpg';
  ...
  Future<void> _removeImage(String imgPath) async {
    ...
    try {
      ...
      // storage’dan sil (hata olsa bile listeden kaldırmak isteyebilirsin ama şimdilik strict)
  ...
  Future<void> _save() async {
    ...
    if (title.isEmpty || city.isEmpty || price == null) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text("Başlık/şehir/fiyat alanlarını doğru doldur.")),
      );
      return;
    }
    ...
    try {
      ...
      await sb.from('products').update({
        ...
        // kolonsa kalsın, yoksa kaldırabilirsin
      ...
      body: ListView(
        ...
        children: [
          ...
          Row(
            children: [
              const Expanded(
                child: Text("Fotoğraflar", style: TextStyle(fontWeight: FontWeight.w800)),
              ),
